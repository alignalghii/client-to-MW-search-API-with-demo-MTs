...
runSearchSlideshow searchphrase = do
    x <- pagination_MT_lazy $ StateT $ wrapperD theServiceLow_lengthy searchphrase
    print x
...
runSearchFirstPage searchphrase = do
    ...
    wrapper theServiceLow_lengthy searchphrase Nothing >>= (putStrLn . showSearchResult)

wrapper, wrapperD :: ServiceLow_lengthy -> String -> PaginationEffect Sroffset IO [Title]
...
runSearchPagedWith info delay searchphrase = do
    ...
    pagination_noMT (wrapper (flip postdelay delay . theServiceLow_lengthy) searchphrase) >>= print
...
wrapper' :: ServiceLow_lengthy -> String -> PaginationEffect Sroffset IO [Title]
...
runSearchPaged_MT_lazy searchphrase = do
    pagination_MT_lazy $ wrapper' theServiceLow_lengthy searchphrase
--}
